...
#### Q9: LLM 額度限制處理 ⚠️ **新增**
**問題描述**: LLM 呼叫有額度限制,超額時應如何處理?

**MVP 決策**:
- ✅ **額度監控**: 實作 LLM 呼叫計數器和額度檢查
- ✅ **失敗分類**: 區分不同類型的失敗原因
  - `QUOTA_EXCEEDED`: 額度用盡 → **暫停系統,通知使用者**
  - `RATE_LIMIT`: 暫時限流 → 等待後重試
  - `NETWORK_ERROR`: 網路錯誤 → 重試
  - `TIMEOUT`: 逾時 → 重試
  - `OTHER`: 其他錯誤 → 降級到隨機選擇
- ✅ **優雅降級**:
  - 額度耗盡時: 暫停所有 LLM 呼叫,世界進入「維護模式」
  - 顯示狀態訊息給觀察者: "系統暫停中,等待管理員處理"
  - 保留世界狀態,不重置
- ✅ **手動恢復**: 提供管理介面讓使用者決定:
  - 選項 A: 增加額度後繼續
  - 選項 B: 切換為降級模式(使用隨機選擇)繼續運行
  - 選項 C: 暫停世界,稍後恢復
